...
(when t  ; Using org-capture now.
  ...
  (defconst tkb-org-blood-pressure   (expand-file-name "~/Repos/tkb-notes/Health/log-blood-pressure.org"))
...
‘org-capture-templates’ to use the new value."
  ...
  (setq org-capture-templates
        `(("X" "EXPERIMENT" entry
          ...
          ("h" "Health" entry
           (file ,tkb-org-health)
           "* %<%F> %^{Title}\n")
          ("p" "Blood pressure" entry
           (file ,tkb-org-blood-pressure)
           "* %<%F %R> - %^{Blood Pressure}, %^{bpm} bpm\n")
          ("n" "Notes" entry
  ...
  (progn
    ...
    (defun tkb-find-org-blood-pressure ()
      "Find Org TKB's Blood-Pressure"
      (interactive)
      (find-file tkb-org-blood-pressure))
    (global-set-key (kbd "C-c k o F p") #'tkb-find-org-blood-pressure)
...
(defun tkb-ruler ()
  ...
  (let* ((width    (window-width)) ; Must be 1 less on old emacs like 18.55
         (width    (if (window-system) width (- width 1)))
         (nheaders (/ width 10))
         (buf      (current-buffer))
         (i 1))
